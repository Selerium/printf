...
/*   Updated: 2022/07/02 21:24:16 by jadithya         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_printstr(char *s, char *flags, int *count, char *add)
{
	...
	else
	{
		if (flags[1] == '1' && flags[0] == '0')
			ft_printzeros(add, count, flags, (int) ft_strlen(s));
		else if (flags[0] == '0')
			ft_printspace(add, count, flags, (int) ft_strlen(s));
		digits = ft_strlen(s);
	}
	write(1, s, digits);
	(*count) += digits;
